...
func (link Link) Save() int64 {
	//#3
	stmt, err := database.Db.Prepare("INSERT INTO Links(Title,Address, UserID) VALUES(?,?,?)") //creates a statement that can be triggered
	if err != nil {
		log.Fatal(err)
	}
	//#4
	res, err := stmt.Exec(link.Title, link.Address, link.User.ID) //run the statement with the arguments that are passed into VALUES(?,?)
	if err != nil {
		log.Fatal(err)
	}
	//#5
